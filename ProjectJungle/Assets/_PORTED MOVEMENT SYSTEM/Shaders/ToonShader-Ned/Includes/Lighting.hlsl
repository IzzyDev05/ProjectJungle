...
void AddAdditionalLights_float(float Smoothness, float3 WorldPos, float3 WorldNormal, float3 WorldView, float MainDiffuse,
                               float MainSpecular, float3 MainColor, out float Diffuse, out float Specular, out float3 Color)
{
    ...
    {
        Light light = GetAdditionalLightForToon(i, WorldPos);
